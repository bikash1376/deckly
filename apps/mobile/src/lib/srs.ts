 ...
 * SM-2 lite.
 *
 * The server owns the real schedule. This exists so the flashcard screen can
 * say "next review in 4 days" the instant a grade is tapped, instead of after
 * a round trip. Both sides run the same function, so the prediction matches
